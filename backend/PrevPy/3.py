...
# Create Flask app manually specifying template & static folders
...
def load_log():
    ...
    with open(LOG_FILE, "r", encoding="utf-8") as f:
        data = json.load(f)
    # Ensure keys exist and data is a dict
...
def log_check(password, flags):
    ...
    entry = {
        ...
        "password": password,      # <-- added for debug
        "username": password if password.lower() in USERS else None,  # marks if it's a username
        "flags": flags
    }
